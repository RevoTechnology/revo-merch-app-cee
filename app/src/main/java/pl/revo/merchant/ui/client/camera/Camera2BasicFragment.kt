...
import pl.revo.merchant.utils.camera2.CompareSizesByArea
import pl.revo.merchant.utils.camera2.ImageSaver
import pl.revo.merchant.utils.camera2.OnListener
import pl.revo.merchant.utils.showToast
...
class Camera2BasicFragment : androidx.fragment.app.Fragment() {
    ...
    private val onImageAvailableListener = ImageReader.OnImageAvailableListener {
        ...
        backgroundHandler?.post(
                ImageSaver(it.acquireNextImage(), file, orientation, object : OnListener<Boolean>{
                    override fun on(arg: Boolean) {
                        ...
                        try {
                            ...
                            val currentOrientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION, 1)
                            val matrix = Matrix()
                        ...
                        activity?.runOnUiThread {
                            ...
                            refreshPhotoButton.visible(true)
                            savePhotoButton.visible(true)

                            previewView.visible(true)
                            previewView.setImageBitmap(imgBitmap)
    ...
    private val captureCallback = object : CameraCaptureSession.CaptureCallback() {
        ...
        private fun process(result: CaptureResult) {
            Log.d("TAG", "captureCallback state=$state")
            when (state) {
        ...
        private fun capturePicture(result: CaptureResult) {
            ...
            val hasAutofocus = activity?.packageManager?.hasSystemFeature("android.hardware.camera.autofocus") == true

            if (afState == null || (afState == CONTROL_AF_STATE_INACTIVE && !hasAutofocus)) {
                captureStillPicture()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        pictureButton.setOnClickListener { lockFocus() }
        refreshPhotoButton.setOnClickListener { refreshButtonClicked() }
        savePhotoButton.setOnClickListener { saveButtonClicked() }
    }

    private fun refreshButtonClicked() {
        refreshPhotoButton.visible(false)
    ...
    private fun saveButtonClicked() {
        ...
        val data = Intent()
        data.putExtra(ExtrasKey.PHOTO_FILE.name, file)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        file = File.createTempFile("IMG_", ".jpg", activity?.cacheDir)
    }
    ...
    private fun setUpCameraOutputs(width: Int, height: Int) {
        ...
        try {
            ...
            activity?.showToast("Camera error ${e.message}")
        }
