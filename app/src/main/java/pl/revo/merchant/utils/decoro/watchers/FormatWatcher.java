...
import pl.revo.merchant.utils.decoro.MaskFactory;
...
public abstract class FormatWatcher implements TextWatcher, MaskFactory {
     ...
     */
    private void installOn(final TextView textView, final boolean initWithMask) {
        if (textView == null) {
    ...
    void refreshMask(@Nullable final CharSequence initialValue) {
    ...
    public void onTextChanged(CharSequence s, int start, int before, int insertedCount) {
        ...
        if (callback != null) {
            callback.onTextChanged(insertedCount);
        }
    }
    ...
    public void afterTextChanged(Editable newText) {
        ...
        if (!formatted.equals(newText.toString())) {
            ...
            if (start == -1 || end == -1) {
                ...
                sb.append(composing);
                sb.append(formatted.substring(end));
                pasted = sb;
            }
            selfEdit = true;
