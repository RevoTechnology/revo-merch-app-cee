...
package pl.revo.merchant.utils.decoro;
...
class SlotsList implements Iterable<Slot>, Parcelable {
    ...
    static SlotsList ofArray(@NonNull Slot[] slots) {
    ...
    SlotsList(@NonNull SlotsList list) {
        if (!list.isEmpty()) {
            Slot previous = null;
            for (Slot slot : list) {
    ...
    boolean checkIsIndex(int position) {
     ...
     */
    Slot insertSlotAt(final int position, @NonNull final Slot slot) {
    ...
    Slot removeSlotAt(int position) {
    ...
    private Slot removeSlot(final Slot slotToRemove) {
    ...
    public void clear() {
        ...
        // iterate back because by default slots will try to pull new value from onNextClick slot
    ...
    @NonNull
    private <T> T[] toArray(@NonNull T[] array) {
    ...
    private static class SlotsIterator implements Iterator<Slot> {
        ...
        SlotsIterator(Slot currentSlot) {
            if (currentSlot == null) {
